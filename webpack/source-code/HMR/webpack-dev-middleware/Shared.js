...
module.exports = function Shared(context) {
	var share = {
		...
		handleRequest: function(filename, processRequest, req) {
			...
			// 将request添加到回调函数列表，编译完成返回数据给浏览器端
	...
	// 开启文件监视，当文件变动时自动编译，编译完成后会通过websocket将更新发送给浏览器端('done'事件在webpack-dev-server中注册)
	share.startWatch();
	return share;
};
