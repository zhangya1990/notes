// Compilation 对象，负责从解析入口开始编译，直到生成编译完成的代码，并挂载在 compilation.assets 上,编译入口方法为 addEntry
"use strict";
...
class Compilation extends Tapable {
	constructor(compiler) {
		...
		// compilation 对象的四种 template，入口 js 会采
		// mainTemplate 负责在 webpack 事件流的 render 事件来触发 Template类 中的renderChunkModules() 生成入口代码
		// chunkTemplate 负责在 webpack 事件流的 render 事件来触发 Template类 中的renderChunkModules() 生成 chunk 代码
		// hotUpdateChunkTemplate 是对热替换模块的一个处理
		// moduleTemplate 是对所有模块进行一个代码生成和包装，如 umd amd 等
		...
		//信号机或执行器，构造函数中有一个初始的使用次数以及一个待执行callback数组。
	...
	// 将当前module添加到 _modules cache modules 数组等
	...
	// 如果为入口module 参数为  module false false null null callback
	...
	// @param dependency 在入口js处为 entry 对象
	_addModuleChain(context, dependency, onModule, callback) {
		...
		// 获取模块的工厂函数 (入口js为 NormalModuleFactory)
		...
		this.semaphore.acquire(() => {
			...
			moduleFactory.create({
				...
				// 将该module信息存储 (_modules modules cache)
	...
	addEntry(context, entry, name, callback) {
		...
		this._addModuleChain(context, entry, (module) => {
			...
			// 如果处理的是入口js，将当前module添加到 entries 数组
			entry.module = module;
